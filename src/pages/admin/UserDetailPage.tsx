...
import { ArrowLeft, Loader as Loader2, Phone, Mail, Calendar, User, Shield, Users, CircleCheck as CheckCircle, CreditCard, Save, Copy, Eye, EyeOff, Key } from 'lucide-react';
import {
  Form,
  FormControl,
  FormField,
              ...
              {userDetail.whatsapp && (
                <div className="flex items-center gap-3">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-auto p-0 text-sm hover:bg-green-50 hover:text-green-700"
                    asChild
                  >
                    <a
                      href={generateWhatsAppUrl(userDetail.whatsapp, `Olá ${userDetail.name}, sou da equipe VitrineTurbo. Como posso ajudar?`)}
                      target="_blank"
                      rel="noopener noreferrer"
                      title={`Conversar com ${userDetail.name} no WhatsApp`}
                    >
                      ...
                      {formatWhatsAppForDisplay(userDetail.whatsapp)}
                    </a>
                  </Button>
                </div>
              )}
