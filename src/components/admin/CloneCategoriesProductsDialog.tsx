...
interface CloneCategoriesProductsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  defaultTargetUserId?: string;
}

export function CloneCategoriesProductsDialog({
  open,
  onOpenChange,
  defaultTargetUserId,
}: CloneCategoriesProductsDialogProps) {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success(`Clonagem concluída com sucesso! Copiado: ${messages.join(', ')}`);
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              <FormLabel>O que deseja clonar?</FormLabel>
